    ...
    for station in stations:
        ...
        if len(df_day_station) > 0:
            ...
            if len(df_day_no_station) >= k:
                ...
                knn_model = neigh.fit(
                    df_day_no_station[['Latitud', 'Longitud', 'time_discriminator']],
                    df_day_no_station[[target_pollutant]]
                )
                prediction = knn_model.predict(df_day_station[['Latitud', 'Longitud', 'time_discriminator']])
...
def create_heat_map_with_date_range(
    ...
    neigh.fit(df_day[['Latitud', 'Longitud']], df_day[[target_pollutant]])
...
def create_animation_features(
    ...
    df_days = df_days[['DateTime', 'Station', 'Latitud', 'Longitud', target_pollutant]]
    ...
    for timestamp in unique_dates:
        ...
        k_neighbors_model.fit(df_day[['Latitud', 'Longitud']], df_day[[target_pollutant]])
        ...
        for index, station in df_day.iterrows():
            ...
            if imputed_col in station and type(station[imputed_col]) == str:
                ...
                bg_color = 'white'
            data = [station['Latitud'], station['Longitud'], station[target_pollutant]]
